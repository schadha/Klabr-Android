<<<<<<< HEAD:src/com/klabr_android/ChatActivity.java
...
public class ChatActivity extends Activity {
	...
	protected void onStop() {
		...
		socket.emit("leave_room", ack, getIntent().getExtras()
				.getString("room"));
	}
	...
	private void initializeComponents() {
		...
		messages = new ArrayList<String>();
		btn_Send.setOnClickListener(socketListener);
		adp = new ArrayAdapter<String>(getBaseContext(), R.layout.custom_list,
				messages);
		// adp.setDropDownViewResource(android.R.layout.simple_dropdown_item_1line);
		listView.setAdapter(adp);
		connectSockets();
		...
		public void onClick(View v) {
			...
			if (message != null && message.length() > 0) {
				...
				try {
					...
					socket.emit("add_message", ack, jObject);
					messages.add(getIntent().getExtras().getString("Username")
							+ " said: " + message);
					adp = new ArrayAdapter<String>(getBaseContext(),
							R.layout.custom_list, messages);
					listView.setAdapter(adp);
...
>>>>>>> FETCH_HEAD:src/com/collabhawk_android/ChatActivity.java
